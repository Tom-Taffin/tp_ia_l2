...
class KdTree:
    ...
    def _up(self,
        ...
        # on calcul la distance entre le point z et le point du noeud current
        explored.add(current)
        # on ajoute le noeud current à l'ensemble explored pour éviter de revenir dessus par la suite
        if current_dist < neighbors[-1][0]: # si la distance est plus petite que la distance la plus grande des k voisins on l'ajoute en conservant neighbors trié et en enlevant la distance max
            i=0
        ...
        # si hyperplane_dist est inferieur à la distance la plus grande des k voisins, il y a potentielement un point plus proche, donc il faut parcourir le sous arbre
        if current.left != None and current.left.node != None and current.left not in explored and hyperplane_dist < neighbors[-1][0]:
            ...
            self._up(z,node_down,distance, k, neighbors, explored)
        if current.parent != None and current.parent not in explored: # on ne monte que si ce n'est pas déja exploré et qu'on est pas à la racine sinon on s'arrete
